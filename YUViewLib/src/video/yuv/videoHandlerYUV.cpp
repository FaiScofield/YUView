                                                         ...
                                                         const PixelFormatYUV &format)
{
  ...
  // Bytes per sample
  const auto bps = (bitDepth > 8) ? 2u : 1u;
  ...
  if (format.getSubsampling() == Subsampling::YUV_422)
  {
    ...
    const int oY =
      (componentOrder == ComponentOrder::YUYV || componentOrder == ComponentOrder::YVYU)
        ? 0
        : 1; // UYVY/VYUY
    const int oU = (componentOrder == ComponentOrder::UYVY)   ? 0
                   : (componentOrder == ComponentOrder::YUYV) ? 1
                   : (componentOrder == ComponentOrder::VYUY) ? 2
                                                              : 3; // YVYU
    const int oV = (componentOrder == ComponentOrder::VYUY)   ? 0
    ...
    if (bps == 1)
    {
      ...
      unsigned rowBytes = w * 2;
      unsigned virtRowBytes = rowBytes;
      if (curFrameSize.hasValidVirtualSize() && curFrameSize.rowPitches[0] > rowBytes)
        virtRowBytes = curFrameSize.rowPitches[0];

      for (int y = 0; y < (int)h; y++)
      {
        const unsigned char *rowSrc = src + y * virtRowBytes;
        for (unsigned x = 0; x < w / 2; x++)
        {
          *dstY++ = rowSrc[oY] >> shiftBits;
          *dstY++ = rowSrc[oY + 2] >> shiftBits;
          *dstU++ = rowSrc[oU] >> shiftBits;
          *dstV++ = rowSrc[oV] >> shiftBits;
          rowSrc += 4;
        }
      }
    }
    else
    {
      ...
      unsigned rowElements = w * 2;
      unsigned virtRowElements = rowElements;
      if (curFrameSize.hasValidVirtualSize() && curFrameSize.rowPitches[0] > rowElements * 2)
        virtRowElements = curFrameSize.rowPitches[0] / 2;

      for (int y = 0; y < (int)h; y++)
      {
        const unsigned short *rowSrc = src + y * virtRowElements;
        for (unsigned x = 0; x < w / 2; x++)
        {
          *dstY++ = rowSrc[oY] >> shiftBits;
          *dstY++ = rowSrc[oY + 2] >> shiftBits;
  ...
  else if (format.getSubsampling() == Subsampling::YUV_444)
  {
    ...
    const int oY = (componentOrder == ComponentOrder::AYUV)   ? 1
                   : (componentOrder == ComponentOrder::VUYA) ? 2
                                                              : 0; // YUV,YVU,YUVA,YVUA
    const int oU =
      (componentOrder == ComponentOrder::YUV || componentOrder == ComponentOrder::YUVA ||
       componentOrder == ComponentOrder::VUYA)
        ? 1
        : 2; // YVU,AYUV,YVUA
    const int oV = (componentOrder == ComponentOrder::YVU || componentOrder == ComponentOrder::YVUA)
                     ? 1
                   : (componentOrder == ComponentOrder::AYUV) ? 3
                   : (componentOrder == ComponentOrder::VUYA) ? 0
                                                              : 2; // YUV,YUVA

    // How many samples to the next sample?
    const int offsetNext =
      (componentOrder == ComponentOrder::YUV || componentOrder == ComponentOrder::YVU ? 3 : 4);
    ...
    if (bps == 1)
    {
      ...
      unsigned rowBytes = w * offsetNext;
      unsigned virtRowBytes = rowBytes;
      if (curFrameSize.hasValidVirtualSize() && curFrameSize.rowPitches[0] > rowBytes)
        virtRowBytes = curFrameSize.rowPitches[0];
    ...
    else
    {
      ...
      unsigned rowElements = w * offsetNext;
      unsigned virtRowElements = rowElements;
      if (curFrameSize.hasValidVirtualSize() && curFrameSize.rowPitches[0] > rowElements * 2)
        virtRowElements = curFrameSize.rowPitches[0] / 2;
...
{
  unpacked_data_u16x4[0] =
    (uint16_t)((packed_data_u8x5[0] >> 0) & 0xFF) | ((packed_data_u8x5[1] & 0x03) << 8);
  unpacked_data_u16x4[1] =
    (uint16_t)((packed_data_u8x5[1] >> 2) & 0x3F) | ((packed_data_u8x5[2] & 0x0F) << 6);
  unpacked_data_u16x4[2] =
    (uint16_t)((packed_data_u8x5[2] >> 4) & 0x0F) | ((packed_data_u8x5[3] & 0x3F) << 4);
  unpacked_data_u16x4[3] =
    (uint16_t)((packed_data_u8x5[3] >> 6) & 0x03) | ((packed_data_u8x5[4] & 0xFF) << 2);
}

std::pair<bool, PixelFormatYUV> unpackYuv10BitToPlanar(const QByteArray     &sourceBuffer,
                                                       QByteArray           &targetBuffer,
                                                       const Size           &size,
                                                       const PixelFormatYUV &format)
{
  ...
  const unsigned       bps       = format.getBitsPerSample(); // 10
  const Subsampling    subsample = format.getSubsampling();
  ...
  // Virtual-size-aware source per-plane strides
  unsigned src_strd_y  = format.getRowPitchForPlane(0, size);
  unsigned src_strd_uv = format.getRowPitchForPlane(1, size);
  ...
  const uint8_t *p_src_y = (uint8_t *)sourceBuffer.data();
  const uint8_t *p_src_u = (uint8_t *)p_src_y + format.getPlaneOffset(1, size);
  const uint8_t *p_src_v = nullptr; // set later
  uint16_t      *p_dst_y = (uint16_t *)targetBuffer.data();
  uint16_t      *p_dst_u = (uint16_t *)((uint8_t *)p_dst_y + h * dst_strd);
  ...
  if (format.getDataLayout() == DataLayout::Interleaved) {
    ...
    {
      unsigned src_row_bytes = size.hasValidVirtualSize() ? size.rowPitches[0] : src_strd * 3;
      ...
      case ComponentOrder::YUV: // load 4 pixels, 12 elements, 15 bytes per iteration
      {
        for (int y = 0; y < h; y++) {
          for (int x = 0, j = 0; x <= w - 4; x += 4, j += 15) {
            const int src_ofs        = y * src_row_bytes + j;
            const int dst_ofs        = y * dst_strd / 2 + x; // /2 for U16 pointer
            uint16_t  unpack_data[4] = {0};
            unpack_data_10bit(p_src_y + src_ofs, unpack_data);
            p_dst_y[dst_ofs + 0] = unpack_data[0];
            p_dst_u[dst_ofs + 0] = unpack_data[1];
            p_dst_v[dst_ofs + 0] = unpack_data[2];
            p_dst_y[dst_ofs + 1] = unpack_data[3];
            unpack_data_10bit(p_src_y + src_ofs + 5, unpack_data);
            p_dst_u[dst_ofs + 1] = unpack_data[0];
            p_dst_v[dst_ofs + 1] = unpack_data[1];
            p_dst_y[dst_ofs + 2] = unpack_data[2];
            p_dst_u[dst_ofs + 2] = unpack_data[3];
            unpack_data_10bit(p_src_y + src_ofs + 10, unpack_data);
            p_dst_v[dst_ofs + 2] = unpack_data[0];
            p_dst_y[dst_ofs + 3] = unpack_data[1];
            p_dst_u[dst_ofs + 3] = unpack_data[2];
      ...
      case ComponentOrder::YVU: // load 4 pixels, 12 elements, 15 bytes per iteration
      {
        for (int y = 0; y < h; y++) {
          for (int x = 0, j = 0; x <= w - 4; x += 4, j += 15) {
            ...
            unpack_data_10bit(p_src_y + src_ofs, unpack_data);
            p_dst_y[dst_ofs + 0] = unpack_data[0];
            p_dst_v[dst_ofs + 0] = unpack_data[1];
            p_dst_u[dst_ofs + 0] = unpack_data[2];
            p_dst_y[dst_ofs + 1] = unpack_data[3];
            unpack_data_10bit(p_src_y + src_ofs + 5, unpack_data);
            p_dst_v[dst_ofs + 1] = unpack_data[0];
            p_dst_u[dst_ofs + 1] = unpack_data[1];
            p_dst_y[dst_ofs + 2] = unpack_data[2];
            p_dst_v[dst_ofs + 2] = unpack_data[3];
            unpack_data_10bit(p_src_y + src_ofs + 10, unpack_data);
            p_dst_u[dst_ofs + 2] = unpack_data[0];
            p_dst_y[dst_ofs + 3] = unpack_data[1];
            p_dst_v[dst_ofs + 3] = unpack_data[2];
      ...
      {
        src_row_bytes = size.hasValidVirtualSize() ? size.rowPitches[0] : src_strd * 4;
        for (int y = 0; y < h; y++) {
          for (int x = 0, j = 0; x < w; x++, j += 5) {
            ...
            uint16_t  unpack_data[4] = {0};
            unpack_data_10bit(p_src_y + src_ofs, unpack_data);
            p_dst_a[dst_ofs] = unpack_data[0];
            p_dst_y[dst_ofs] = unpack_data[1];
            p_dst_u[dst_ofs] = unpack_data[2];
      ...
      case ComponentOrder::VUYA: // load 1 pixel, 4 elements, 5 bytes per iteration
      {
        ...
        for (int y = 0; y < h; y++) {
          for (int x = 0, j = 0; x < w; x++, j += 5) {
            ...
            uint16_t  unpack_data[4] = {0};
            unpack_data_10bit(p_src_y + src_ofs, unpack_data);
            p_dst_v[dst_ofs] = unpack_data[0];
            p_dst_u[dst_ofs] = unpack_data[1];
            p_dst_y[dst_ofs] = unpack_data[2];
      ...
      case ComponentOrder::YUVA: // load 1 pixel, 4 elements, 5 bytes per iteration
      {
        ...
        for (int y = 0; y < h; y++) {
          for (int x = 0, j = 0; x < w; x++, j += 5) {
            ...
            uint16_t  unpack_data[4] = {0};
            unpack_data_10bit(p_src_y + src_ofs, unpack_data);
            p_dst_y[dst_ofs] = unpack_data[0];
            p_dst_u[dst_ofs] = unpack_data[1];
            p_dst_v[dst_ofs] = unpack_data[2];
            p_dst_a[dst_ofs] = unpack_data[3];
          }
        }
      } break;
    ...
    {
      unsigned src_row_bytes = size.hasValidVirtualSize() ? size.rowPitches[0] : src_strd * 2;
      ...
      const int oY = (format.getComponentOrder() == ComponentOrder::YUYV ||
                      format.getComponentOrder() == ComponentOrder::YVYU)
                       ? 0
                       : 1; // UYVY/VYUY
      const int oU = (format.getComponentOrder() == ComponentOrder::UYVY)   ? 0
                     : (format.getComponentOrder() == ComponentOrder::YUYV) ? 1
                     : (format.getComponentOrder() == ComponentOrder::VYUY) ? 2
                                                                            : 3; // YVYU
      const int oV = (format.getComponentOrder() == ComponentOrder::VYUY)   ? 0
                     : (format.getComponentOrder() == ComponentOrder::YVYU) ? 1
                     : (format.getComponentOrder() == ComponentOrder::UYVY) ? 2
                                                                            : 3; // YUYV
      ...
      for (int y = 0; y < h; y++) {
        for (int x = 0, j = 0; x < w / 2; x++, j += 5) {
          ...
          p_dst_y[dst_ofs_y + 1] = unpack_data[oY + 2];
          p_dst_u[dst_ofs_c]     = unpack_data[oU];
          p_dst_v[dst_ofs_c]     = unpack_data[oV];
        }
      }
      return {true, newFormat};
    }
    ...
    for (int y = 0; y < h; y++) {
      for (int x = 0, j = 0; x <= w - 4; x += 4, j += 5) {
        const int src_ofs_y = y * src_strd_y + j;
    ...
    case Subsampling::YUV_420: { // yuv420p 10bit bytepacking
      if (compOrder == ComponentOrder::YUV) {
        p_src_v = (uint8_t *)p_src_y + format.getPlaneOffset(2, size);
      } else if (compOrder == ComponentOrder::YVU) {
        p_src_v = p_src_u;
        p_src_u = (uint8_t *)p_src_y + format.getPlaneOffset(2, size);
      } else {
        LOGE(
      ...
      for (int y = 0; y < h / 2; y++) {
        for (int x = 0, j = 0; x <= w / 2 - 4; x += 4, j += 5) {
          const int src_ofs_c = y * src_strd_uv + j;
          const int dst_ofs_c =
            y * dst_strd / 4 + x; // /4 for U16 pointer & half width of chroma plane
          unpack_data_10bit(p_src_u + src_ofs_c, p_dst_u);
    ...
    case Subsampling::YUV_444: { // yuv444p 10bit bytepacking
      ...
      for (int y = 0; y < h; y++) {
        for (int x = 0, j = 0; x <= w - 4; x += 4, j += 5) {
          const int src_ofs = y * src_strd_uv + j;
          const int dst_ofs = y * dst_strd / 2 + x; // /2 for U16 pointer
    ...
    case Subsampling::YUV_444: // nv30
    {
      ...
      for (int y = 0; y < h; y++) {
        for (int x = 0, j = 0; x <= w - 4; x += 4, j += 5) {
          const int src_ofs_c      = y * src_strd_uv + j * 2;
          const int dst_ofs        = y * dst_strd / 2 + x;     // /2 for U16 pointer
          ushort    unpack_data[4] = {0};
          unpack_data_10bit(p_src_u + src_ofs_c, unpack_data);
          p_dst_u[dst_ofs + 0] = unpack_data[oU];
          p_dst_v[dst_ofs + 0] = unpack_data[oV];
          p_dst_u[dst_ofs + 1] = unpack_data[oU + 2];
          p_dst_v[dst_ofs + 1] = unpack_data[oV + 2];
          unpack_data_10bit(p_src_u + src_ofs_c + 5, unpack_data);
          p_dst_u[dst_ofs + 2] = unpack_data[oU];
          p_dst_v[dst_ofs + 2] = unpack_data[oV];
          p_dst_u[dst_ofs + 3] = unpack_data[oU + 2];
                           ...
                           const ConversionSettings &conversionSettings)
{
  ...
  const unsigned char *cleanSrc   = (unsigned char *)sourceBuffer.data();
  QByteArray           cleanBuffer;
  if (curFrameSize.hasValidVirtualSize() && format.isPlanar())
  {
    unsigned rowPitchY  = format.getRowPitchForPlane(0, curFrameSize);
    unsigned heightY    = format.getHeightForPlane(0, curFrameSize);
    unsigned rowPitchUV = format.getRowPitchForPlane(1, curFrameSize);
    unsigned heightUV   = format.getHeightForPlane(1, curFrameSize);

    unsigned logicalRowBytesY  = (bps > 8) ? w * 2 : w;
    unsigned logicalRowBytesUV = (bps > 8) ? (w / format.getSubsamplingHor()) * 2 : (w / format.getSubsamplingHor());
    if (format.isUVInterleaved())
      logicalRowBytesUV *= 2;

    bool hasPadding = (rowPitchY > logicalRowBytesY || heightY > h ||
                       rowPitchUV > logicalRowBytesUV || heightUV > (h / format.getSubsamplingVer()));

    if (hasPadding)
    {
      unsigned cleanPlaneOffset = 0;
      unsigned nrPlanes         = format.getNrPlanes();

      for (unsigned i = 0; i < nrPlanes && i < 4; i++)
      {
        unsigned rowPitch  = format.getRowPitchForPlane(i, curFrameSize);
        unsigned height    = format.getHeightForPlane(i, curFrameSize);
        unsigned planeOffset = format.getPlaneOffset(i, curFrameSize);

        unsigned logicalWidth = (i == 0) ? w : (w / format.getSubsamplingHor());
        unsigned logicalRowBytes = (bps > 8) ? logicalWidth * 2 : logicalWidth;
        if (i > 0 && format.isUVInterleaved())
          logicalRowBytes *= 2;
        unsigned logicalHeight = (i == 0) ? h : (h / format.getSubsamplingVer());

        cleanPlaneOffset += logicalRowBytes * logicalHeight;
        cleanBuffer.resize(cleanPlaneOffset);

        const unsigned char *srcPlane = (unsigned char *)sourceBuffer.data() + planeOffset;
        unsigned char       *dstPlane = (unsigned char *)cleanBuffer.data() + cleanPlaneOffset - logicalRowBytes * logicalHeight;

        for (unsigned y = 0; y < logicalHeight; y++)
          std::memcpy(dstPlane + y * logicalRowBytes, srcPlane + y * rowPitch, logicalRowBytes);
      }

      cleanSrc = (unsigned char *)cleanBuffer.data();
    }
  }

  const unsigned char *restrict srcBase = cleanSrc;
      ...
      format.getSubsampling() == Subsampling::YUV_400)
  {
        ...
        format.getSubsampling() == Subsampling::YUV_400)
    {
      ...
      const unsigned char *restrict srcY = srcBase;
    ...
    else
    {
      ...
      const unsigned char *restrict srcC = srcBase + srcOffset;
...
bool convertYUVToImage(const QByteArray         &sourceBuffer,
                       QImage                   &outputImage,
...
QLayout *videoHandlerYUV::createVideoHandlerControls(bool isSizeAndFormatFixed)
{
  ...
  connect(ui.lumaOffsetSpinBox,
          ...
          &videoHandlerYUV::slotYUVControlChanged);
  connect(
    ui.lumaInvertCheckBox, &QCheckBox::stateChanged, this, &videoHandlerYUV::slotYUVControlChanged);
...
void videoHandlerYUV::setSrcPixelFormat(PixelFormatYUV format, bool emitSignal)
{
  ...
  // Validate and normalize virtual size
  if (frameSize.hasValidVirtualSize())
  {
    format.validateAndNormalizeVirtualSize(frameSize);
  }
...
void videoHandlerYUV::loadFrame(int frameIndex, bool loadToDoubleBuffer)
{
  ...
  {
    QImage newImage;
    ok = convertYUVToImage(this->currentFrameRawData,
                           newImage,
                           this->srcPixelFormat,
                           this->frameSize,
...
yuv_t videoHandlerYUV::getPixelValue(const QPoint &pixelPos) const
{
  ...
  if (auto predefinedFormat = format.getPredefinedFormat()) {
    ...
    // Get virtual size parameters
    const unsigned rowPitchY    = format.getRowPitchForPlane(0, frameSize); // byte
    const unsigned heightY      = format.getHeightForPlane(0, frameSize);   // pixel
    const unsigned rowPitchUV   = format.getRowPitchForPlane(1, frameSize); // byte
    const unsigned heightUV     = format.getHeightForPlane(1, frameSize);   // pixel
    const uint64_t offsetPlane1 = format.getPlaneOffset(1, frameSize);
    const uint64_t offsetPlane2 = format.getPlaneOffset(2, frameSize);

    const int subsampleH = format.getSubsamplingHor();
    const int subsampleV = format.getSubsamplingVer();
    const int chromaRow  = pixelPos.y() / subsampleV;
    const int chromaCol  = pixelPos.x() / subsampleH;

    // Luma first
    const unsigned char *restrict srcY = (unsigned char *)currentFrameRawData.data();

    if (format.isBytePacking()) {
      /* Handle 10-bit bytepacking planar formats */
      if (format.getBitsPerSample() == 10) {

        // Calculate Y position
        const int yRow       = pixelPos.y();
        const int yCol       = pixelPos.x();
        const int ySrcOffset = yRow * rowPitchY + (yCol / 4) * 5;
        ...
        if (format.getSubsampling() != Subsampling::YUV_400) {
          ...
          if (format.isUVInterleaved()) {
            ...
            const unsigned char *restrict srcUVA = srcY + offsetPlane1;
            const int          uvSrcOffset       = chromaRow * rowPitchUV + (chromaCol * 2 / 4) * 5;
            uint16_t           uvUnpack[4]       = {0};
            unpack_data_10bit(srcUVA + uvSrcOffset, uvUnpack);
            const int uvPairIdx = (chromaCol % 2) * 2;
            value.U = uvUnpack[uFirst ? uvPairIdx : uvPairIdx + 1];
            value.V = uvUnpack[uFirst ? uvPairIdx + 1 : uvPairIdx];
          }
          else {
            const unsigned char *restrict srcU = uFirst ? srcY + offsetPlane1 : srcY + offsetPlane2;
            const unsigned char *restrict srcV = uFirst ? srcY + offsetPlane2 : srcY + offsetPlane1;

            const int uSrcOffset = chromaRow * rowPitchUV + (chromaCol / 4) * 5;
            uint16_t  uUnpack[4] = {0};
            unpack_data_10bit(srcU + uSrcOffset, uUnpack);
            value.U = uUnpack[chromaCol % 4];

            const int vSrcOffset = chromaRow * rowPitchUV + (chromaCol / 4) * 5;
        ...
        LOGE("videoHandlerYUV::getPixelValue Unsupported bit depth {} of bytepacking format",
             format.getBitsPerSample());
        return value;
      }
    }
    else {
      /* unpacked planar case */
      const unsigned int Bps = (format.getBitsPerSample() + 7) / 8;
      const unsigned int offsetCoordY = pixelPos.y() * rowPitchY / Bps + pixelPos.x();
      value.Y = getValueFromSource(srcY, offsetCoordY, format.getBitsPerSample(),
                                   format.isBigEndian(), format.getPaddingInfo());
      ...
      if (format.getSubsampling() != Subsampling::YUV_400) {
        ...
        if (format.isUVInterleaved()) {
          // U, V (and alpha) are interleaved
          const unsigned char *restrict srcUVA = srcY + offsetPlane1;
          const unsigned int offsetCoordUV     = chromaRow * rowPitchUV / Bps + chromaCol * 2;

          value.U = getValueFromSource(srcUVA, offsetCoordUV + (uFirst ? 0 : 1),
                                       format.getBitsPerSample(), format.isBigEndian(),
                                       format.getPaddingInfo());
          value.V = getValueFromSource(srcUVA, offsetCoordUV + (uFirst ? 1 : 0),
                                       format.getBitsPerSample(), format.isBigEndian(),
                                       format.getPaddingInfo());
        }
        else {
          ...
          const int chromaRow     = pixelPos.y() / subsampleV;
          const int chromaCol     = pixelPos.x() / subsampleH;
          const int offsetCoordUV = chromaRow * rowPitchUV / Bps + chromaCol;

          value.U = getValueFromSource(srcU, offsetCoordUV, format.getBitsPerSample(),
                                       format.isBigEndian(), format.getPaddingInfo());
          value.V = getValueFromSource(srcV, offsetCoordUV, format.getBitsPerSample(),
                                       format.isBigEndian(), format.getPaddingInfo());
        }
      }
    }
    ...
    const unsigned rowPitch = format.getRowPitchForPlane(0, frameSize);
    ...
    if (format.getSubsampling() == Subsampling::YUV_422) {
      ...
      if (format.isBytePacking() && format.getBitsPerSample() == 10) {
        ...
        const unsigned srcStride     = rowPitch;
        auto           offsetInInput = pixelPos.y() * srcStride + (pixelPos.x() / 2) * 5;
        const unsigned char *restrict src =
        ...
        const unsigned offsetCoordinate4Block = (rowPitch * pixelPos.y() + (pixelPos.x() / 2 * 4)) *
                                                (format.getBitsPerSample() > 8 ? 2 : 1);
        const unsigned char *restrict src =
          (unsigned char *)currentFrameRawData.data() + offsetCoordinate4Block;

        value.Y =
          getValueFromSource(src, (pixelPos.x() % 2 == 0) ? oY : oY + 2, format.getBitsPerSample(),
                             format.isBigEndian(), format.getPaddingInfo());
      ...
      if (format.isBytePacking() && format.getBitsPerSample() == 10) {
        ...
        const int srcStride               = rowPitch;
        const int pixelX                  = pixelPos.x();
        const int pixelY                  = pixelPos.y();
        const int srcOffset               = pixelY * srcStride + (pixelX / 4) * 15;
        const unsigned char *restrict src = (unsigned char *)currentFrameRawData.data() + srcOffset;
        ...
        if (packing == ComponentOrder::YUV || packing == ComponentOrder::YVU) {
          ...
          // Layout: [Y₀,U₀,V₀,Y₁] [U₁,V₁,Y₂,U₂] [V₂,Y₃,U₃,V₃]
          // Unpack all 3 blocks so we can index by value position
          uint16_t allValues[12] = {0};
          unpack_data_10bit(src + 0 * 5, &allValues[0]);
          unpack_data_10bit(src + 1 * 5, &allValues[4]);
          unpack_data_10bit(src + 2 * 5, &allValues[8]);
          const int baseIdx = posInBlock * 3;
          value.Y = allValues[baseIdx];
          value.U = allValues[baseIdx + (packing == ComponentOrder::YVU ? 2 : 1)];
          value.V = allValues[baseIdx + (packing == ComponentOrder::YVU ? 1 : 2)];
        ...
        const int offsetNext =
          ...
          (format.getBitsPerSample() > 8 ? 2 : 1);
        const int offsetSrc               = (rowPitch * pixelPos.y() + pixelPos.x()) * offsetNext;
                                            ...
                                            const bool       markDifference)
{
  ...
  LOGD("videoHandlerYUV::calculateDifference frame idx item 0 {} - item 1 {}",
       frameIdxItem0,
       frameIdxItem1);
